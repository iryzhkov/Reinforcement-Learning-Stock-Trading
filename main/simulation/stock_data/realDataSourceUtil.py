...
def getStockDataInformation(data, stock_symbol):
    if hasStockDownloadedFor(data, stock_symbol):
        return data['stored_stocks'][stock_symbol];
    else:
        return None;

def updateStockDataInformation(stock_symbol, new_information):
    data['stored_stocks'][stock_symbol] = new_information;
    updateFile();

def canPerformRequest():
    checkRequestCountAndDate()
    return data['quandl_requests_today'] <= QUANDL_MAX_REQUESTS_PER_DAY 
